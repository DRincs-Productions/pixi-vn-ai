import { narration, stepHistory, type NarrationHistory } from "@drincs/pixi-vn";
import {
    ai,
    DialogTemplate,
    initAI,
...
function renderDialogueLine(item: NarrationHistory, index: number) {
    ...
    const text = Array.isArray(item.dialogue.text) ? item.dialogue.text.join("") : item.dialogue.text;
    return (
        <p key={index}>
...
export default function App() {
    ...
    const [scene, setScene] = useState("The throne room, late at night, lit by torches.");
    ...
    function handlePreviewPrompt() {
        setPromptPreview(promptBuilder.build(DialogTemplate, request, buildOptions()));
    }
        ...
        <div style={{ display: "flex", gap: 24, padding: 24, fontFamily: "sans-serif" }}>
            <section style={{ flex: "0 0 280px" }}>
                ...
                <p style={{ color: "#666", fontSize: 14 }}>
                    Played automatically on load via Pixi'VN's narration, this is the narrative
                    history `ai.dialog.generate` can inject when `history: true`.
                </p>
            ...
            <section style={{ flex: 1, maxWidth: 640 }}>
                ...
                <fieldset style={{ marginBottom: 16 }}>
                    ...
                    <button type="button" onClick={handleLoadModel} disabled={loadingModel || modelReady}>
                        {modelReady ? "Model ready" : loadingModel ? "Loading…" : "Load model"}
                    </button>
                    {loadingModel && <p style={{ fontSize: 13, color: "#666" }}>{loadingProgress}</p>}
                </fieldset>
                ...
                <fieldset style={{ marginBottom: 16 }}>
                    ...
                    <br />
                    <label>
                        Style: <input value={style} onChange={(e) => setStyle(e.target.value)} />
                    </label>{" "}
                    <label>
                        Language:{" "}
                ...
                <div style={{ marginBottom: 16 }}>
                    ...
                    <button type="button" onClick={handleGenerate} disabled={!modelReady || generating}>
